...
Ngày hôm qua, chúng ta đã tìm hiểu về phép trừ hai con trỏ và phép so sánh hai con trỏ cùng trỏ tới hai phần tử trong một mảng. Chúng ta đã rút ra được kết luận rằng kết quả của hai phép toán trên liên quan đến vị trí tương đối của các phần tử có trong mảng. Vậy tại sao lại có mối liên hệ này? Chúng ta sẽ cùng tìm hiểu.
...
``` 
Ở đây, chương trình in ra địa chỉ của `a[0]` và `a[1]` ở trong mảng đã cho. Ngoài ra nó in kết quả của phép tính `q-p` chúng ta đã nói đến ở hôm qua. 
...
### 2. Sử dụng con trỏ trong xử lí mảng
Các phép toán liên quan đến con trỏ có thể giúp chúng ta truy cập từng phần tử của mảng bằng việc lặp lại phép toán cộng 1 đơn vị vào biến con trỏ.
Xét một chương trình tính tổng một mảng như sau:
```c
#include <stdio.h>
int sum(int a[], int n)
{
...
```
Kết quả cho thấy chương trình đã tính đúng tổng của các số đã cho trong mảng. Khi đó ta có thể thấy được chương trình có thể thực hiện nhiệm vụ được giao, mặc dù có điểm kỳ lạ trong cú pháp.
